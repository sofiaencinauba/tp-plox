...
RSpec.describe AST::Statement do
  describe 'var declaration' do
    it 'creates a variable declaration statement' do
      var_decl = AST::VarDeclaration.new(Token.new(TokenType::IDENTIFIER, 'x'), AST::Literal.new(5))
      expect(var_decl.name.lexeme).to eq('x')
  ...
  describe 'function declaration' do
    it 'creates a function declaration statement' do
      func_decl = AST::FunctionDeclaration.new(
        Token.new(TokenType::IDENTIFIER, 'myFunc'),
        [Token.new(TokenType::IDENTIFIER, 'a'), Token.new(TokenType::IDENTIFIER, 'b')],
        AST::BlockStatement.new([AST::Literal.new(1)])
      )
      expect(func_decl.name.lexeme).to eq('myFunc')
      expect(func_decl.params.map(&:lexeme)).to eq(%w[a b])
      expect(func_decl.body).to be_a(AST::BlockStatement)
      expect(func_decl.body.statements.first).to be_a(AST::Literal)
      expect(func_decl.body.statements.first.value).to eq(1)
    end
  end
  describe 'expression statement' do
    it 'creates an expression statement' do
      expr_stmt = AST::ExpressionStatement.new(AST::Literal.new(10))
  ...
  describe 'print statement' do
    it 'creates a print statement' do
      print_stmt = AST::PrintStatement.new(AST::Literal.new('Hello'))
  ...
  describe 'block statement' do
    it 'creates a block statement' do
      block_stmt = AST::BlockStatement.new([AST::Literal.new(1), AST::Literal.new(2)])
  ...
  describe 'return statement' do
    it 'creates a return statement' do
      return_stmt = AST::ReturnStatement.new(AST::Literal.new(42))
  ...
  describe 'if statement' do
    it 'creates an if statement' do
      if_stmt = AST::IfStatement.new(AST::Literal.new(true), AST::Literal.new('then'), AST::Literal.new('else'))
